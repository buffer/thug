...
class TestThugLogging:
    ...
    def test_add_code_snippet(self):
        ...
        tag_hex = thug_logging.add_code_snippet(
            self.js, "Javascript", "Contained_Inside"
        )
        assert not tag_hex

        log.ThugOpts.code_logging = True
        assert not thug_logging.add_code_snippet(
            "var", "Javascript", "Contained", check=True
        )
    ...
    def test_add_shellcode_snippet(self):
        tag_hex = thug_logging.add_shellcode_snippet(
            "sample", "Assembly", "Shellcode", "Static Analysis"
        )
        assert tag_hex
    ...
    def test_log_exploit_event(self, caplog):
        ...
        thug_logging.log_exploit_event(
            "https://www.example.com", "module", "sample-description"
        )
